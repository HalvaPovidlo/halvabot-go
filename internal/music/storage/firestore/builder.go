...
type oldSong struct {
	...
	Service      item.ServiceName `firestore:"service,omitempty" csv:"service,omitempty" json:"service,omitempty"`
...
func parseSongDoc(doc *firestore.DocumentSnapshot) (item.Song, error) {
	var s item.Song
	err := doc.DataTo(&s)
	if err != nil {
		var old *oldSong
		err = doc.DataTo(&old)
		if err != nil {
			return item.Song{}, errors.Wrap(err, "unable to marshal song data")
		}
		s = buildNewSong(old)
	}
	return s, nil
...
func buildNewSong(s *oldSong) item.Song {
	return item.Song{
		Title:        s.Title,
		URL:          s.URL,
		Service:      s.Service,
